        ...
        int contadorRep = 0;
        	try{
                for(int i = 1; i < controlador; i++){
                    Filme tmp = Filme.clonador(arrayFilmes[i]);
                    int j = i - 1;
                    while ( ((j >= 0) && (arrayFilmes[j].getDataLancamentoDate()).compareTo(tmp.getDataLancamentoDate()) > 0)){
                        arrayFilmes[j + 1] = Filme.clonador(arrayFilmes[j]);
                        j--;
                        contadorRep ++;
        
                    }
                    arrayFilmes[j + 1] = Filme.clonador(tmp);
                    contadorRep ++;
                }
