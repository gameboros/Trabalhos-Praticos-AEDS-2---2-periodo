...
class TP03Q08 {
    public static void main(String[] args) throws FileNotFoundException {
       ...
       boolean houveTroca = true;
        for(int i = 0; i < controlador - 1 && houveTroca; i++){
	        houveTroca = false;
	        for (int j = 0; j < controlador -(i+1); j++)

                if (arrayFilmes[j].getDuracao() > arrayFilmes[j+1].getDuracao()){
                    Filme aux = Filme.clonador(arrayFilmes[j]);
                    arrayFilmes[j] = Filme.clonador(arrayFilmes[j+1]);
                    arrayFilmes[j + 1] = Filme.clonador(aux);
			        houveTroca = true;
		        }

                else if(arrayFilmes[j].getDuracao() == arrayFilmes[j+1].getDuracao()){
                    if((arrayFilmes[j].getNome()).compareTo(arrayFilmes[j + 1].getNome())>   0){
                        Filme aux = arrayFilmes[j];
                        arrayFilmes[j] = arrayFilmes[j + 1];
                        arrayFilmes[j + 1] = aux;
                    }
                }
        }
