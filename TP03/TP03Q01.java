...
class TP03Q01 {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        listafilmes.reordenarSelection();
...
class Filme {
    ...
    public String getDataLancamento() {
        String newdate = "";
        try{
            newdate = new SimpleDateFormat("dd/MM/yyyy").format(dataLancamento);
        } catch (NullPointerException npe){
            newdate = "";
        }
        return newdate;
...
class Lista {
    ...
    public void reordenarSelection(){
        try{
            for (int i =0; i< n-1; i++){
                int min_idx=i;
                for (int j=i+1;j<n;j++){
                    if((((array[j]).getTituloOriginal()).compareTo((array[min_idx]).getTituloOriginal()))<0){
                        min_idx=j;
                    }
                    Filme tmp = Filme.clonador(array[min_idx]);
                    array[min_idx] = Filme.clonador(array[i]);
                    array[i] = Filme.clonador(tmp);
                }
            }
