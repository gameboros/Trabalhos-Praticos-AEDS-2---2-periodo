...
class TP03Q05 {
    public static void main(String[] args) throws FileNotFoundException {
        ...
        for(int i = controlador/2 - 1; i >= 0; i--)
        {
            turnHeap(arrayFilmes, controlador, i);
        }
    
        for(int i = controlador - 1; i >= 0; i--)
        {
            Filme temp = Filme.clonador(arrayFilmes[0]);  
            arrayFilmes[0] = Filme.clonador(arrayFilmes[i]);  
            arrayFilmes[i] = Filme.clonador(temp);  
    
            turnHeap(arrayFilmes, i, 0);
        }
    

        for(int i = 0; i < controlador; i++)
        {
            arrayFilmes[i].imprimir();
        }
    ...
    static void turnHeap(Filme a[], int n, int i){
        int largest = i; 
        int left = 2 * i + 1;  
        int right = 2 * i + 2;  
        if ( (left < n && a[left].getGenero().compareTo(a[largest].getGenero()) > 0))   
            largest = left;  
        if ( (right < n && a[right].getGenero().compareTo(a[largest].getGenero()) > 0) )  
            largest = right;  
        if (largest != i) {  
            Filme temp = Filme.clonador(a[i])  ;
            a[i] = a[i];Filme.clonador(a[largest]);  
            a[largest] = Filme.clonador(temp);  
